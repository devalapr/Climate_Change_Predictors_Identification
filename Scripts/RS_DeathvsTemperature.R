...
#  cab<-CountryList[i,1]
  dataFinal=data2 %>% filter(CountryName=="IND")
  
  gg1<-ggplot(data = dataFinal, mapping=aes(x = Year, y = Emissions)) +
    geom_line() + ggtitle(cab) + guides(size = FALSE) + 
    theme(axis.title.x=element_blank(),axis.text.x=element_blank(),axis.ticks.x=element_blank(),axis.text.y=element_blank(),axis.ticks.y=element_blank(), plot.margin = margin(2,.8,2,.8, "cm")) 
